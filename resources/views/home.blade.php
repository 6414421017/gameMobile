...
@section('content')
    <div class="carousel relative container mx-auto" style="max-width:1600px;">
        <div class="carousel-inner relative overflow-hidden w-full">
            <!--Slide 1-->
            @foreach ($dataRecom as $item)
                <input class="carousel-open" type="radio" id="carousel-{{ $item->id }}" name="carousel" aria-hidden="true"
                    hidden="" checked="checked">
                <div class="carousel-item absolute opacity-0" style="height:50vh;">
                    <div class="block h-full w-full mx-auto flex pt-6 md:pt-0 md:items-center bg-cover bg-right"
                        style="background-image: url('{{ asset($item->recom_img) }}');">

                        <div class="container mx-auto">
                            <div
                                class="flex flex-col w-full lg:w-1/2 md:ml-16 items-center md:items-start px-6 tracking-wide">
                                <p class="text-white text-5xl my-4 drop-shadow-md">{{ $item->recom_detail }}</p>
                            </div>
                        </div>
        ...
        <div class="container mx-auto flex items-center flex-wrap pt-4 pb-12">
            ...
            @foreach ($dataGames as $item)
                <div class="w-full md:w-1/3 xl:w-1/4 p-6 flex flex-col">
                    <a href="{{route('detail_game',$item->id)}}" data-modal-show="default-modal">
                        <img class="hover:grow hover:shadow-lg w-full" src="{{ asset($item->game_img_logo) }}" >
                        <div class="pt-3 flex items-center justify-between">
                            <p class="">{{ $item->game_name }}</p>
                        ...
                        <p class="pt-1 text-gray-900">{{ $item->game_price }}</p>
                    </a>
                </div>
            @endforeach
    ...
    <section class="bg-white py-8">
        ...
        <div class="container py-8 px-6 mx-auto">
            ...
            <p class="mt-8 mb-8">เกี่ยวกับเว็บไซต์นี้</p>

            <p class="mb-8">เว็ยไซต์นี้เป็นเว็บแนะนำเกมที่เป็น The best ทั้งเกมที่ไม่มีคนรู้จักและเกมที่มีคนรู้จักอยู่แล้ว 
                ตรงส่วนด้านบนจะเป็นการแสดงหน้าโชว์เกมที่ไม่ควรพลาดในแต่ละรายสัปดาห์ และในตรงส่วนเนื้อหาจะเป็นที่แสดงเกม 
                และเว็บไซต์นี้ยังสามารถเพิ่มเกมที่ต้องการได้ด้วย เพื่อแสดให้ผู้อื่นเข้ามาเห็นโดยรวม และจากนั้นพอเวลาผ่านเกมเกมจะแสดง
                ในระบบเป็นจำนวนมาก เว็บไซต์จึงมีหน้าที่สามารถลบข้อมูลได้ เพื่อเหลือไว้แต่เกมที่ควรแนะนำจริงๆ
            </p>
                ...
                <div class="flex w-full lg:w-1/2 lg:justify-end lg:text-right mt-6 md:mt-0">
                    <div class="px-3 md:px-0">
                        ...
                        <div class="w-full flex items-center py-4 mt-0">
                            ...
                            <a href="https://www.facebook.com/Gift.wchp" class="mx-2" target="_blank">
                                <svg class="w-6 h-6 fill-current" viewBox="0 0 24 24">
                                    <path
